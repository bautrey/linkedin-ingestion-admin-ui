...
router.get('/api/status', async (req, res) => {
    try {
        ...
        try {
            const ingestionResponse = await axios.get(`${apiUrl}/api/v1/ingestion-jobs`, {
                timeout: 5000,
                headers: {
            ...
            if (ingestionResponse.data && Array.isArray(ingestionResponse.data)) {
                allJobs = [...allJobs, ...ingestionResponse.data.slice(0, 50)];
            }
        } catch (apiError) {
            logger.warn('Could not fetch ingestion jobs:', apiError.message);
        }
