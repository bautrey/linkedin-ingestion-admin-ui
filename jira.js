...
class JiraClient {
        ...
        try {
            ...
            const response = await this.client.post('/search', {
                ...
                fields: [
                    ...
                    // Processing fields - these need to be discovered
                    // 'customfield_XXXXX', // Fit Status
                    // 'customfield_YYYYY', // Fit Score
                ]
            });
    ...
    async updateCandidate(key, fields) {
        ...
        try {
            ...
            const fieldMapping = {
                ...
                // Processing fields - TODO: Replace with actual field IDs once discovered
                fitStatus: 'customfield_XXXXX', // Placeholder for Fit Status field
                fitScore: 'customfield_YYYYY'   // Placeholder for Fit Score field
            };
    ...
    checkTokenExpiration() {
        if (!this.tokenExpires) {
            logger.warn('Jira token expiration date not configured');
            return { status: 'unknown', daysRemaining: null, expired: false };
        }

        const expirationDate = new Date(this.tokenExpires);
        const now = new Date();
        const daysRemaining = Math.ceil((expirationDate - now) / (1000 * 60 * 60 * 24));
        const expired = daysRemaining <= 0;
        const warningRequired = daysRemaining <= this.warningDays && daysRemaining > 0;

        const status = {
            status: expired ? 'expired' : (warningRequired ? 'warning' : 'valid'),
            daysRemaining,
            expired,
            warningRequired,
        ...
        if (expired) {
            logger.error(`Jira API token has EXPIRED! Expired ${Math.abs(daysRemaining)} days ago. Please update JIRA_API_TOKEN in environment variables.`);
        } else if (warningRequired && this.warningEnabled) {
            logger.warn(`Jira API token will expire in ${daysRemaining} days (${expirationDate.toDateString()}). Please prepare to update JIRA_API_TOKEN.`);
        }
     ...
     * @returns {Object} Token status with user-friendly messages
     */
    getTokenStatus() {
        const status = this.checkTokenExpiration();
