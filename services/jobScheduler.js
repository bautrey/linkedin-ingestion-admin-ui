...
class JobScheduler extends EventEmitter {
    constructor() {
        ...
        // Jobs will be initialized asynchronously by server.js
    }
    ...
    async initializeJobs() {
        try {
            logger.info('🔧 DEBUG: Starting job initialization from database...');
            const jobDefinitions = await jobDefinitionService.getAllEnabledJobs();
            logger.info(`🔧 DEBUG: Found ${jobDefinitions.length} job definitions:`, jobDefinitions.map(j => ({name: j.name, handler: j.handler_function, enabled: j.enabled})));
            
            for (const jobDef of jobDefinitions) {
                logger.info(`🔧 DEBUG: Processing job '${jobDef.name}' with handler '${jobDef.handler_function}'`);
                const handler = this.getJobHandler(jobDef.handler_function);
                if (handler) {
                    logger.info(`🔧 DEBUG: Handler found for '${jobDef.name}', registering job...`);
                    this.registerJob(
                        jobDef.name,
                        jobDef.schedule,
                        handler,
                    ...
                    logger.info(`🔧 DEBUG: Successfully registered job '${jobDef.name}'`);
                } else {
                    logger.warn(`🔧 DEBUG: No handler found for job '${jobDef.name}' with function '${jobDef.handler_function}'`);
                }
            }
            
            logger.info(`🔧 DEBUG: Job scheduler initialized with ${jobDefinitions.length} jobs from database. Total registered jobs: ${this.jobs.size}`);
            logger.info(`🔧 DEBUG: Registered job names:`, Array.from(this.jobs.keys()));
        } catch (error) {
    ...
    getJobHandler(handlerName) {
        ...
        const handlerMap = {
            'processRecentCandidates': () => {
                ...
                return candidateProcessingJob.processRecentCandidates.bind(candidateProcessingJob);
            },
            'processOlderCandidates': () => {
                const candidateProcessingJob = require('./candidateProcessingJob');
                return candidateProcessingJob.processOlderCandidates.bind(candidateProcessingJob);
            }
        };

        logger.info(`🔧 DEBUG: getJobHandler called for '${handlerName}'`);
        const factory = handlerMap[handlerName];
        if (factory) {
            logger.info(`🔧 DEBUG: Factory function found for '${handlerName}'`);
            return factory;
        } else {
            logger.error(`🔧 DEBUG: No factory function found for '${handlerName}'`);
            return null;
        }
    }
    ...
    async executeJob(name) {
        ...
        while (retryCount <= job.options.maxRetries) {
            try {
                logger.info(`🔧 DEBUG: About to execute job handler for '${name}'`);
                logger.info(`🔧 DEBUG: Handler factory type:`, typeof job.handler);
                
                // Call factory function to get actual handler
                const actualHandler = job.handler();
                logger.info(`🔧 DEBUG: Actual handler type:`, typeof actualHandler);
                logger.info(`🔧 DEBUG: Actual handler function:`, actualHandler.toString().substring(0, 200));
                
                // Now call the actual handler
                result = await actualHandler();
                logger.info(`🔧 DEBUG: Job handler completed for '${name}', result:`, result);
                error = null;
    ...
    async triggerJob(name) {
        logger.info(`🔧 DEBUG: triggerJob called for '${name}'`);
        logger.info(`🔧 DEBUG: Available jobs:`, Array.from(this.jobs.keys()));
        
        const job = this.jobs.get(name);
        if (!job) {
            logger.error(`🔧 DEBUG: Job '${name}' not found in jobs map`);
            throw new Error(`Job '${name}' not found`);
        }

        logger.info(`🔧 DEBUG: Job '${name}' found, executing...`);
    ...
    async getJobStatus(name = null) {
        logger.info(`🔧 DEBUG: getJobStatus called with name: ${name}`);
        logger.info(`🔧 DEBUG: Current jobs in memory: ${this.jobs.size}`, Array.from(this.jobs.keys()));
        logger.info(`🔧 DEBUG: Current job statuses: ${this.jobStatus.size}`, Array.from(this.jobStatus.keys()));
        
        if (name) {
            const status = this.jobStatus.get(name);
            const job = this.jobs.get(name);
            logger.info(`🔧 DEBUG: Looking for job '${name}' - status found: ${!!status}, job found: ${!!job}`);
